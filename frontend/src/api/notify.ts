...
/** 统计用户未读通知数 */
export function unreadCount(userId: string | number): Promise<number> {
  return request<number>({ url: '/notify/unread-count', method: 'get', params: { userId } })
}

/** 标记单条已读 */
export function markRead(id: number, userId: string | number): Promise<boolean> {
  return request<boolean>({ url: `/notify/${id}/read`, method: 'post', params: { userId } })
}

/** 标记全部已读，返回更新条数 */
export function markAllRead(userId: string | number): Promise<number> {
