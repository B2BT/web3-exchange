...
- **P2.3 Staking/Earn**：✅ 已落地（2026-08-07）。`exchange-staking` 模块（端口 8112）：质押产品(活期/锁仓)/质押/赎回/年化收益每日结算(credit 入账,requestId 幂等)。前端 /staking 页(产品卡片+质押表单+持仓+收益流水)。契约 docs/staking-domain.md。实测:锁仓质押/活期赎回/收益结算136986301入账/幂等 全链路通过。
- **P2.4 风控引擎**：✅ 已落地（2026-08-07）。`exchange-risk` 模块（端口 8114）：下单风控(滑点/单笔/日累计限额)/反钓鱼码/提现二次验证/异常登录日志。三域接入:order下单风控拦截(超限REJECTED)、chain提现反钓鱼码、auth登录日志。前端 /risk 账户安全页。契约 docs/risk-domain.md。实测:滑点/限额拦截✓ 反钓鱼码✓ 登录日志✓ 全链路通过。
- **P2.5 Admin B**：✅ 已落地（2026-08-07）。`exchange-admin` 扩展：公告管理(CRUD+发布)/交易对管理(t_symbol 上牌停牌)/管理员审计日志/服务健康监控(内部上报)。前端 admin 后台 4 页。契约 docs/admin-b-domain.md。实测:公告发布✓ 交易对停牌✓ 审计自动记录✓ 健康上报✓ 全链路通过。
